...
// NUR die frischeste vollstaendige Frame PRO id funken. Ein Echtzeit-Setpoint-
...
// bei F_des=0). Deshalb: alles Anstehende parsen, aber je id nur die letzte Frame
// senden. Pro id (nicht global): im Schwarmbetrieb schickt die GCS pro Tick eine
// Frame je Drohne direkt hintereinander -- ein globales "nur die letzte" wuerde
// sonst systematisch alle Drohnen ausser der letzten verhungern lassen.
static void serial_pump() {
    static uint8_t buf[gcs::SIZE];
    static int idx = 0;
    static uint8_t st = 0;                       // 0=HUNT0, 1=HUNT1, 2=FILL
    uint8_t  latest[16][gcs::SIZE];              // frischeste Frame je id (BCD 0..15)
    bool     have[16] = {};
        ...
        switch (st) {
            ...
            default:
                ...
                if (idx >= gcs::SIZE) {                          // vollstaendige Frame:
                    uint8_t fid = buf[gcs::off::ID];             // CRC prueft forward_frame
                    if (fid < 16) {
                        for (int i = 0; i < gcs::SIZE; ++i) latest[fid][i] = buf[i];
