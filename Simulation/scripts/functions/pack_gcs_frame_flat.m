...
%   [sync|id|Bus_Cmd_flat(float32)|estop|ack|crc8]. Spiegelt gcs_frame_flat.hpp
%   (build) und ist zugleich die Byte-Spec fuer die Simulink-Serial-Send-Seite:
%   die GCS muss genau dieses Layout erzeugen.
%
%   cmd: struct mit mocap_pos(3), q_ext(4), p_ref(3), v_ref(3), a_ref(3),
%        j_ref(3), s_ref(3), yaw_ref(3), estop(0/1/2), ack(0/1).
%        Feldreihenfolge == Bus_Cmd_flat (setup_buses.m).
