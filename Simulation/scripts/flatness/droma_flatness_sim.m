...
% droma_flatness_sim  Flachheitsbasierter Folgeregler (exakte Linearisierung) +
%   Schub-Skalenschaetzer mit DROMA-Parametern, standalone, z-up, toolbox-frei.
%   Schub +R(:,3), F = zeta1 spez. Schub.
%   Aktuatorpfad: Regler-Wrench [F_N;tau] -> Gamma_inv -> w_i^2_cmd ->
%   sqrt/Saettigung -> Motor-PT1 -> Gamma -> echte [F;tau].
%
%   I_ctrl_scale : Faktor auf die Regler-Traegheit, die Strecke behaelt I.
%   thrust_scale : aerodyn. Schub-Download auf die gelieferte Schubkraft.
%   gamma_khat   : Adaptionsgain Schub-Skalenschaetzer, F_cmd = zeta1/k_hat,
%                  k_hat_dot = gamma*(s_mess - zeta1), s_mess = zB'*(a_ist + g).
%   tau_m        : Motor-Zeitkonstante.
...
sim_root = fileparts(fileparts(here)); % scripts/flatness -> scripts -> Simulation
...
m    = quadcop.m;
I    = quadcop.J; % Traegheit der Strecke
I_c  = I_ctrl_scale * I; % Regler-Traegheit
D    = diag([0 0 0]);
Gam  = quadcop.Gamma; % [F_N; tau] = Gamma * w_i^2
Gaminv = quadcop.Gamma_inv; % Mixer
...
% Gam enthaelt bereits quadcop.k_thrust, thrust_scale wirkt zusaetzlich.
ts_eff = thrust_scale * quadcop.k_thrust;
w_rotor_max = 28597 * 2*pi/60; % ~2994 rad/s aero-Envelope
...
% velocity-innovation-Schaetzer: Luenberger-v vs Modellpraediktion
...
for i=1:Nt-1
  ...
  switch meas_src
    ...
    case 'mocap' % 100-Hz-Pos -> doppelt differenziert -> TP -> auf zB projiziert
      ...
      s_meas = s_meas_hold;
    case 'mocap_av' % Beschl. aus einmaliger Diff. der Beobachter-v
      if tdisc(i) - t_moc_prev >= Tm - 1e-9
      ...
      s_meas = s_meas_hold;
    case 'mocap_vi' % velocity-innovation: Luenberger-v vs Modellpraediktion
      ...
      s_meas = zeta1; % neutralisiert das generische Update
    otherwise % 'truth'
      s_meas = s_truth;
  end

  % --- Flachheits-Regelgesetz ---
  a = -g*eZ + zeta1*R(:,3);
  ...
  % --- Strecke mit Motor-PT1 integrieren ---
